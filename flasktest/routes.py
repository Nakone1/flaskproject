...
from flasktest.forms import LoginForm, RegistationForm
...
def register():
	...
	if form.validate_on_submit():
		...
		db.session.add(user)
		db.session.commit()
		flash(f'Аккаунт создан! Можете войти','Успешно')
...
@login_required
def raspisanie():
    conn = sqlite3.connect('flasktest/site.db')
    c = conn.cursor()
    c.execute("SELECT * FROM schedule")
    content = c.fetchall()
    return render_template('schedule.html', content=content)
